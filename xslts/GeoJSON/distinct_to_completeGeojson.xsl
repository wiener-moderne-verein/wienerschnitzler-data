...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:result-document href="../../../data/editions/geojson/wienerschnitzler_distinctPlaces.geojson" method="text">
            <xsl:text>{</xsl:text>
            ...
            <xsl:for-each select="$listPlaceGesamt/tei:place[mam:koordinaten-vorhanden(@xml:id) and tei:listEvent[1]/tei:event]">
                ...
                <xsl:value-of select="mam:macht-punkt($place, 'complete', descendant::tei:event/@when, descendant::tei:event/@when)"/>
    ...
    <xsl:function name="mam:koordinaten-vorhanden" as="xs:boolean">
        ...
        <xsl:choose>
            <xsl:when test="$lookup/tei:location[@type = 'coords'][1]/tei:geo[not(normalize-space(.) = '')]">
